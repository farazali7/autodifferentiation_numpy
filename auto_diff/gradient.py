...
from .comp_graph import Node, NodeQueue
import numpy as np
import derivatives as derivs
from .utilities import correct_deriv
...
def gradient(node):
    ...
    while len(queue) > 0:
        ...
        curr_grads[curr_node.operand_a_name] = correct_deriv(curr_node.operand_a,
                                                             curr_grads[curr_node.operand_a_name] + next_grads[0])
        ...
        if curr_node.operand_b is not None:
            curr_grads[curr_node.operand_b.name] = correct_deriv(curr_node.operand_b,
                                                                 curr_grads[curr_node.curr_node.operand_b.name] + next_grads[1])
